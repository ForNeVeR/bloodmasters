...
public class WindowBorder
{
    ...
    public unsafe void CreateGeometry()
    {
        ...
        vertices = new VertexBuffer(Direct3D.d3dd, sizeof(TLVertex) * verts.Count,
            Usage.WriteOnly, TLVertex.Format, Pool.Default);

        // Lock vertex buffer
        var vertsarray = vertices.Lock<TLVertex>(0, verts.Count);

        // Fill the vertex buffer
        verts.ToArray().CopyTo(vertsarray);
