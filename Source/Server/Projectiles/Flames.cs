...
public class Flames : Projectile
{
    ...
    public Flames(Vector3D start, Vector3D vel, Client source) : base(start, vel, source)
    {
        ...
        damagetime = SharedGeneral.currenttime + DAMAGE_INTERVAL;
    ...
    public override void Process()
    {
        ...
        // Time to do damage?
        if(damagetime <= SharedGeneral.currenttime)
        {
            ...
            foreach(Client c in Host.Instance.Server.clients)
            {
                   ...
                   ((c != Source) || (travellength > FREE_TRAVEL_LENGTH)))
                {
                    ...
                    if((c.LastFlameTime + DAMAGE_MAX_INTERVAL) < SharedGeneral.currenttime)
                    {
                        ...
                        if(distance < damagedistance)
                        {
                            ...
                            if(damage >= 2f)
                            {
                                ...
                                c.LastFlameTime = SharedGeneral.currenttime;
