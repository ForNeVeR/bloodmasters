...
namespace CodeImp.Bloodmasters.Server
{
	public class Flag : Item
	{
		...
		public Flag(Thing t) : base(t)
		{
			...
			if(Global.Instance.Server.GameType != GAMETYPE.CTF) this.Temporary = true;
		}
		...
		public override void Pickup(Client c)
		{
			...
			else
			{
				...
				{
					// Return the flag
					Global.Instance.Server.BroadcastReturnFlag(c, this);
					this.Return();
				}
				else
				{
					...
					if(c.Carrying is Flag)
					{
						...
						// SCORE!
						Global.Instance.Server.BroadcastScoreFlag(c, carryflag);
						carryflag.Return();
						c.AddToScore(1);
					}
				}
			...
			else
			{
				...
				if(dpos.LengthSq() > 0.01f)
				{
					...
					if(returntime < SharedGeneral.currenttime)
					{
						...
						Global.Instance.Server.BroadcastReturnFlag(null, this);
						this.Return();
					}
				}
			}
