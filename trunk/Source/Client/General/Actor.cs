		...
		public override void Render()
		{
			...
			if(sector.VisualSector.InScreen && !disposed)
			{
				...
				if(showring)
				{
					...
					Direct3D.SetDrawMode(DRAWMODE.NADDITIVEALPHA);
					Direct3D.d3dd.RenderState.ZBufferWriteEnable = false;
					Direct3D.d3dd.RenderState.TextureFactor = General.TeamColor(team, 0.8f);
				...
				if(!dead)
				{
					...
					Direct3D.d3dd.Transform.World = Matrix.Multiply(legs_scalerotate, apos);
					Direct3D.d3dd.Transform.Texture0 = DirectionCellMatrix(DirFromAngle(aimangle, 0, 16));
				...
				// Apply matrices
				Direct3D.d3dd.Transform.World = Matrix.Multiply(torso_scalerotate, apos);
				Direct3D.d3dd.Transform.Texture0 = DirectionCellMatrix(DirFromAngle(aimangle, 0, 16));
