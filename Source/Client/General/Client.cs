...
public class Client
{
    ...
    public void SwitchWeapon(WEAPON weaponid, bool silent)
    {
        ...
        if(allweapons[(int)weaponid] != null)
        {
            ...
            {
                // Make sound
                if(actor != null) SoundSystem.PlaySound("weaponswitch.wav", actor.Position);
    ...
    public void SetPowerupCountdown(int count, bool fired)
    {
            ...
            (powerupfired == false) && (fired == true))
        {
            ...
            SoundSystem.PlaySound("countdownnuke.wav");
        }
    ...
    public void ProcessStaticPowerup()
    {
        ...
        if (actor.Sector.VisualSector.InScreen && (General.random.Next(10) < 2))
        {
            ...
            SoundSystem.PlaySound("lightning_e.wav", actor.Position);
        ...
        if (actor != null)
        {
            ...
            if (actor.Sector.VisualSector.InScreen)
            {
                ...
                if (variations > 0)
                {
                    ...
                    SoundSystem.PlaySound("death" + (int)method + "var" + var + ".wav", actor.Position);
                }
            }

            // Spawn floor blood here
            if ((method != DEATHMETHOD.QUIET)
                && (actor.HighestSector != null)
                && (actor.HighestSector.Material != (int)SECTORMATERIAL.LIQUID))
            {
                FloorDecal.Spawn(
                    sector: actor.HighestSector,
                    nx: actor.Position.x,
                    ny: actor.Position.y,
                    textureset: FloorDecal.blooddecals,
                    permanent: false,
                    resize: true,
                    small: false);
            }
    ...
    public void DestroyActor(bool silent)
    {
        ...
        if (actor != null)
        {
            ...
            if (!silent)
            {
                ...
                SoundSystem.PlaySound("playerleave.wav", actor.Position);

                // Determines team color
                switch (team)
                {
                    case TEAM.NONE:
                        teamcolor = General.ARGB(1f, 0f, 0.6f, 0f);
                        break;
                    case TEAM.RED:
                        teamcolor = General.ARGB(1f, 1f, 0f, 0f);
                        break;
                    case TEAM.BLUE:
                        teamcolor = General.ARGB(1f, 0f, 0.6f, 1f);
                        break;
                    default:
                        teamcolor = Color.White.ToArgb();
                        break;
                }

                // Add particles
                for (int i = 0; i < 20; i++)
                    General.arena.p_dust.Add(
                        actor.Position + Vector3D.Random(General.random, 2.5f, 2.5f, 2f) + new Vector3D(0f, 0f, 8f),
                ...
                for (int i = 0; i < 20; i++)
                    General.arena.p_dust.Add(actor.Position + Vector3D.Random(General.random, 4f, 4f, 10f),
                        Vector3D.Random(General.random, 0.02f, 0.02f, 0.1f) + actor.State.vel * 0.2f,
                        Color.NavajoWhite.ToArgb());
                for (int i = 0; i < 10; i++)
                ...
                for (int i = 0; i < 10; i++)
                    General.arena.p_dust.Add(
                        actor.Position + Vector3D.Random(General.random, 1f, 1f, 2f) + new Vector3D(0f, 0f, 10f),
                        Vector3D.Random(General.random, 0.02f, 0.02f, 0.1f) + actor.State.vel * 0.2f,
                        General.ARGB(1f, 1.0f, 0.8f, 0.6f));
                for (int i = 0; i < 30; i++)
                    General.arena.p_smoke.Add(actor.Position + Vector3D.Random(General.random, 3f, 3f, 0f),
                        Vector3D.Random(General.random, 0.02f, 0.02f, 0.02f) + actor.State.vel * 0.1f,
                        General.ARGB(1f, 0.6f, 0.6f, 0.6f));
            }
    ...
    public void SpawnActor(NetMessage msg)
    {
        ...
        if (!start)
        {
            ...
            if (this.IsLocal)
                SoundSystem.PlaySound("playerspawn.wav");
            else if (actor.Sector.VisualSector.InScreen)
                SoundSystem.PlaySound("playerspawn.wav", actor.Position);
            ...
            if (this.IsLocal || actor.Sector.VisualSector.InScreen)
            {
                ...
                {
                    case TEAM.NONE:
                        teamcolor = General.ARGB(1f, 0.5f, 1f, 0.5f);
                        break;
                    case TEAM.RED:
                        teamcolor = General.ARGB(1f, 1f, 0.4f, 0.4f);
                        break;
                    case TEAM.BLUE:
                        teamcolor = General.ARGB(1f, 0.4f, 0.5f, 1f);
                        break;
                    default:
                        teamcolor = Color.White.ToArgb();
                        break;
    ...
    public void Teleport(NetMessage msg)
    {
        ...
        SoundSystem.PlaySound("teleport.wav", oldpos);
        SoundSystem.PlaySound("teleport.wav", newpos);
        ...
        if(hurtlevel > 0)
        {
            ...
            if((this.actor != null) && (hurtlevel > 0))
            {
                ...
                if((hurtsound == null) || (hurtsound.Playing == false))
                {
                    ...
                    // Make hurt sound
                    hurtsound = SoundSystem.GetSound("hurt" + hurtlevel.ToString(CultureInfo.InvariantCulture) + ".wav", true);
