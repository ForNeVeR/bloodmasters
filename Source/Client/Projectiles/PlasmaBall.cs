		...
		public override void Destroy(Vector3D atpos, bool silent, Client hitplayer)
		{
			...
			if((silent == false) && (sector != null))
			{
				...
				if(sector.VisualSector.InScreen)
				{
					// Spawn particles
					for(int i = 0; i < 3; i++)
						General.arena.p_magic.Add(decalpos + Vector3D.Random(General.random, 1f, 1f, 1f), Vector3D.Random(General.random, 0.1f, 0.1f, 0.2f), General.ARGB(1f, 0f, 0.6f, 1f));
					for(int i = 0; i < 2; i++)
						General.arena.p_magic.Add(decalpos + Vector3D.Random(General.random, 1f, 1f, 1f), Vector3D.Random(General.random, 0.1f, 0.1f, 0.2f), -1);
				}
			}
			// Silent destroy
			else if(sector != null)
			{
				...
				if((SECTORMATERIAL)sector.Material == SECTORMATERIAL.LIQUID)
				{
					...
					if(sector.VisualSector.InScreen)
					{
						...
						{
							case LIQUID.WATER: FloodedSector.SpawnWaterParticles(atpos, new Vector3D(0f, 0f, 0.5f), 3); break;
							case LIQUID.LAVA: FloodedSector.SpawnLavaParticles(atpos, new Vector3D(0f, 0f, 0.5f), 3); break;
						}
					}
				}
			}
